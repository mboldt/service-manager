	...
	Describe("List", func() {
		Context("when no criteria is used", func() {
			It("builds simple query for entity and its labels", func() {
				...
				Expect(executedQuery).Should(Equal(trim(`SELECT visibilities.*,
															   ...
															   WHERE visibility_id IN
																   (SELECT visibility_id
																	...
																	WHERE (visibility_labels.key = ?
																		   AND visibility_labels.val = ?))) visibility_labels ON visibilities.id = visibility_labels.visibility_id;`)))
																   ...
																   (SELECT visibility_id
																	...
																	WHERE (visibility_labels.key = ?
																	  ...
																	  OR (visibility_labels.key = ?
																		  AND visibility_labels.val != ?))) visibility_labels ON visibilities.id = visibility_labels.visibility_id
															WHERE visibilities.id::text != ?
															  AND visibilities.service_plan_id::text NOT IN (?, ?, ?)
															  AND (visibilities.platform_id::text = ?
																   OR visibilities.platform_id IS NULL);`)))
	...
	Describe("Delete", func() {
		Context("when no criteria is used", func() {
			It("builds query to delete all entries", func() {
				...
				Expect(executedQuery).To(Equal(`DELETE FROM visibilities;`))
			})
		})
															...
															JOIN
															  (SELECT *
															   ...
															   WHERE visibility_id IN
																   (SELECT visibility_id
																	FROM visibility_labels
																	WHERE (visibility_labels.key = ? AND visibility_labels.val = ?)
																	  OR (visibility_labels.key = ? AND visibility_labels.val IN (?, ?)))) visibility_labels 
		...
		Context("when field criteria is used", func() {
			It("builds query with field criteria", func() {
				...
				Expect(executedQuery).Should(Equal(trim(`DELETE FROM visibilities WHERE visibilities.id::text = ?;`)))
				Expect(queryArgs).To(HaveLen(1))
				Expect(queryArgs[0]).Should(Equal("1"))
			})
		...
		Context("when returning clause is used", func() {
			It("builds query with specified fields fields", func() {
				...
				Expect(executedQuery).To(Equal(`DELETE FROM visibilities RETURNING id,service_plan_id;`))
			})
			...
			It("builds query with *", func() {
				...
				Expect(executedQuery).To(Equal(`DELETE FROM visibilities RETURNING *;`))
			})
				...
				Expect(executedQuery).Should(Equal(trim(`DELETE
															...
															WHERE visibilities.id::text != ?
															  AND visibilities.service_plan_id::text NOT IN (?,
																											 ...
																											 ?)
															  AND (visibilities.platform_id::text = ?
																   OR visibilities.platform_id IS NULL) RETURNING *;`)))
				Expect(queryArgs).To(HaveLen(12))
				Expect(queryArgs[0]).Should(Equal("left1"))
				Expect(queryArgs[1]).Should(Equal("right1"))
				Expect(queryArgs[2]).Should(Equal("left2"))
