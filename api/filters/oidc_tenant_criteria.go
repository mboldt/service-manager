...
//TODO
// add component tests in delete_list for the support of delete by label query
...
// add task for e2e tests and define scenarios
// sm-dev adopt and add new config properties in the dev env
...
type OIDCTenantCriteriaFilter struct {
	// ClientID is the ID of the OAuth client, the tokens of which are issued for users with multi tenant access
	ClientID string
...
func (f *OIDCTenantCriteriaFilter) Run(request *web.Request, next web.Handler) (*web.Response, error) {
	...
	if clientIDFromToken != f.ClientID {
